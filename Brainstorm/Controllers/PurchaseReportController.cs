    ...
    public class PurchaseReportController : Controller
    {
        ...
        public ActionResult GetPurchaseReportByDate(DateTime startDate, DateTime endDate)
        {
            ...
            var q = (from PurPro in PurchaseProducts
                     ...
                     select new PurchaseReport { Id = pur.Id, Code = Prod.Code, Name = Prod.Name, Category = Cat.Name, AvailableQuantity =GetAvailable(Prod.Id,pur.Date), CostPrice = PurPro.UnitPrice, MRP = PurPro.MRPPrice, Profit = PurPro.MRPPrice - PurPro.UnitPrice }).ToList();
            ViewBag.x = q;
